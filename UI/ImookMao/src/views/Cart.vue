...
export default {
  ...
  computed: {
    totalPrice: function() {
      ...
      this.cartList.forEach((item) => {
        if(item.checked === 1) {
          totlePrice += parseFloat(item.salePrice) * parseInt(item.productNum);
        }
      });
      return totlePrice;
    },
    checkAllFlag: function() {
      return this.cartList.length === this.checkedCount.length;
    },
  ...
  methods: {
    ...
    toggleCheckAll () {
      let checkedVal, checkAllFlag;

      checkAllFlag = !this.checkAllFlag;
      this.cartList.forEach((item) => {
        item.checked = 1;
      });
      checkedVal = checkAllFlag ? 1 : -1;
      this.cartList.forEach((item) => {
